...
/**
 * Поп-ап про ресурсное состояние. Показывается на «точках пересборки» (см. сервер,
 * risk/resourceState.ts): начало торгового дня и конец часового перерыва после закрытой
 * сделки — то есть ровно тогда, когда снова можно входить в рынок. Ответ хранится на
 * сервере, поэтому на каждую точку спросят один раз, с любого устройства.
 *
 * Ничего не блокирует: «нет» — это не запрет торговли, а напоминание (NotResourcefulNotice).
 * Закрыть поп-ап можно только ответом — вопрос на один тап.
 */
        ...
        <div className="flex flex-col gap-1.5">
          <h2 className="text-base font-medium text-ink">
            {askReason === "cooldown"
              ...
              : "Ты сегодня в ресурсном состоянии?"}
          </h2>
          <p className="text-xs text-slate-500">
            {askReason === "cooldown"
              ? "Перерыв после сделки закончился — сверимся перед новым входом."
              : "Отвечаем один раз за день — торговля из ресурса и торговля из пустоты дают разный результат."}
          </p>
        </div>
...
/** Напоминание после ответа «нет» — висит рядом с формой открытия до следующего вопроса. */
  ...
  return (
    <div className="mx-4 flex flex-col gap-1 rounded-2xl border border-amber-200 bg-amber-50 p-4">
      ...
      <p className="text-xs text-amber-700">
        По твоей последней отметке. Лучший план на сегодня — наполнить себя ресурсом, а не
        торговать.
      </p>
    </div>
  );
