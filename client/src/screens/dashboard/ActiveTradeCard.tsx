...
const RR_PRESETS = ["1/0.9", "1/1", "1/1.5", "1/1.9", "1/2", "1/3", "1/4"];
/**
...
export function ActiveTradeCard({
  ...
  return (
    <div className="mx-4 flex flex-col gap-4 rounded-2xl border border-line bg-card p-4 shadow-sm">
      ...
      {/* Сервер уже запустил досверку с биржей (см. api/dashboard.ts) — обычно карточка
          сама обновится закрытой сделкой в течение ~15–30 секунд. Кнопка ниже остаётся
          аварийным путём, если сверка не смогла достучаться до BingX. */}
      {trade.positionFlat && (
        <p className="rounded-lg bg-amber-50 px-3 py-2 text-xs text-amber-700">
          Позиция уже закрылась на бирже (SL/TP). Сверяем с BingX и фиксируем результат
          автоматически — обычно до полуминуты. Если карточка не обновилась, нажмите
          «Закрыть сделку».
        </p>
      )}
