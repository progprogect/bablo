...
const RESET = 7;
const NIGHT = DEFAULT_NIGHT_START_HOUR; // 1

test("DEFAULT_NIGHT_START_HOUR — 01:00 МСК", () => {
  assert.equal(DEFAULT_NIGHT_START_HOUR, 1);
});

test("isLocalNight: 01:00–06:59 — ночь, иначе нет", () => {
  // 00:30 UTC+3 — ещё не ночь
  assert.equal(isLocalNight(new Date("2026-07-13T21:30:00Z"), NIGHT, RESET, TZ), false);
  // 01:00 UTC+3
...
test("getNightStartAtOrBefore / getNextNightStartAt при старте 01:00", () => {
  // 15:00 UTC+3 → предыдущие 01:00 того же календарного дня
  const afternoon = new Date("2026-07-14T12:00:00Z");
  assert.equal(getNightStartAtOrBefore(afternoon, NIGHT, TZ).toISOString(), "2026-07-13T22:00:00.000Z");
  assert.equal(getNextNightStartAt(afternoon, NIGHT, TZ).toISOString(), "2026-07-14T22:00:00.000Z");

  // 02:00 UTC+3 → текущая ночь началась в 01:00
  const night = new Date("2026-07-13T23:00:00Z");
  assert.equal(getNightStartAtOrBefore(night, NIGHT, TZ).toISOString(), "2026-07-13T22:00:00.000Z");
  assert.equal(getNextNightStartAt(night, NIGHT, TZ).toISOString(), "2026-07-14T22:00:00.000Z");
});

test("isDayTradeIntoNight: дневная сделка после 01:00 — да; открытая ночью — нет", () => {
