 ...
 *
 * Отдельно от этой автоматики существуют РУЧНЫЕ блокировки часа (hourBlockReview.ts,
 * 22.09.2026): час закрывает сам пользователь, и снимается такая блокировка только своей
 * проверкой месячного винрейта. Здесь они участвуют лишь как «час уже закрыт» — правила
 * 3 и 4 к ним не применяются.
 */
...
export type BlockedHourState = {
  ...
  /**
   * Час закрыт решением пользователя, а не расчётом (см. hourBlockReview.ts). Такие
   * блокировки снимаются только своей проверкой месячного винрейта, поэтому автоматика
   * их не трогает — иначе гистерезис открыл бы час на первой же закрытой сделке.
   */
  manual?: boolean;
};
...
export function decideHourBlocks(input: HourBlockDecisionInput): HourBlockDecision {
  ...
  for (const entry of currentlyBlocked) {
    // Ручная блокировка живёт по своим правилам (hourBlockReview.ts) — ни гистерезис,
    // ни «час пропал из статистики» её не снимают.
