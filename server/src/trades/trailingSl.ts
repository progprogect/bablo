 ...
 * (0 = точка входа, 1 = +1R и т.д.). ОТРИЦАТЕЛЬНЫЙ slR — стоп остаётся в убытке, но ближе
 * к входу: так работает лестница тейка 2R (правило от 14.09.2026), где стоп ужимается
 * вдвое, а не уводится в безубыток. Уровни считаются от ИСХОДНОГО риска
...
const TRAILING_LADDERS: Record<string, TrailingLevel[]> = {
  /**
   * Тейк 2R (правило от 14.09.2026): на 1.4R стоп ужимается с −1R до −0.5R. Цель другая,
   * чем у лестниц ниже: не забрать часть прибыли, а вдвое урезать убыток, если цена
   * развернулась почти дойдя до цели. В безубыток стоп здесь НЕ уводится — до тейка
   * остаётся всего 0.6R, и стоп на входе выбивал бы сделку прямо перед целью.
   */
  "1/2": [{ triggerR: 1.4, slR: -0.5 }],
  "1/3": [
    { triggerR: 1.9, slR: 0 },
    { triggerR: 2.25, slR: 1 },
  ],
