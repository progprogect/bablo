...
/**
 * Пресеты, отображаемые в сетке R месячной карточки — только до 1/3
 * (сокращалось: 1/6 → 1/4 21.08.2026 → 1/3 10.09.2026 вслед за выбором тейка).
 * Столбцы выше стояли почти пустыми: при R/R 1/5+ обязательна частичная фиксация ≤1/3,
 * и такая сделка ложится в сетку по пресету partial.
 *
 * Сетка УЖЕ словаря RR_PRESETS осознанно: сделка, достигшая 4R, получает бакет "1/4"
 * (matchRrPreset) и просто не отрисовывается. Если убрать 1/4 ещё и из RR_PRESETS,
 * она откатится на плановый пресет и попадёт в столбец 3R — то есть 4R-сделка была бы
 * показана как 3R.
 */
export const STATS_GRID_PRESETS = ["1/1", "1/1.5", "1/2", "1/3"] as const;
