...
export const trades = pgTable("trades", {
  ...
  /**
   * Когда сервер поджал TP до 1/1 на ночь (дневная сделка активна после 01:00 МСК) —
   * идемпотентность: повторно не трогаем.
   */
  nightTpAppliedAt: timestamp("night_tp_applied_at", { withTimezone: true }),
  /**
   * Последний применённый уровень трейлинг-лестницы SL для полных тейков 1/2, 1/3 и 1/4
