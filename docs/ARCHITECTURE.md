...
- **Поиск исполнившегося SL/TP вынесен в `realtime/filledOrder.ts`** (без зависимостей от
  `trades/`, чтобы не было цикла service ↔ reconcile). Им пользуются все три пути, где
  нужно понять причину закрытия: сверка по ACCOUNT_UPDATE, ручное закрытие при уже
  закрытой позиции и разовая сверка при старте сервера (сделка активна в БД, позиции на
  бирже нет — событие потеряно за время рестарта/деплоя). Без последних двух сделка,
  реально закрытая по SL/TP, записывалась как `external` и выпадала из статистики
  и дневных лимитов (правка от 13.08.2026).
