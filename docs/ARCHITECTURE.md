...
- **Баг найден и исправлен 16.07.2026**: `GET /openApi/swap/v2/trade/order` у BingX (как и
  ответ `placeOrder`) оборачивает данные в `{ data: { order: {...} } }`, а не отдаёт поля
  ордера прямо в `data`. `getOrderStatus` в `bingx/client.ts` этого не учитывал — `status`
  всегда был `undefined`, из-за чего REST-сверка (`reconcilePositionFlat`) никогда не могла
  определить, какой из SL/TP сработал, и всегда помечала закрытие как `"external"`, даже
  когда сделка реально закрылась по стопу/тейку. Это искажало все инсайты и месячную
  статистику, завязанные на `closeReason`. Для восстановления уже закрытых сделок есть
  админ-эндпоинт `POST /admin/reclassify-trades` (`trades/reclassify.ts`) — повторно
  запрашивает статус сохранённых `bingxOrderIds` и чинит `closeReason`/результат там, где
  BingX ещё отдаёт данные по ордеру.
